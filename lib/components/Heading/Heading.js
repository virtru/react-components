 ...
 * @prop {Boolean} nospace Indicates whether heading should have spacing (margin)
 * @prop {Node} children
 * @return {*}
 */
const Heading = ({ size, rank, children, color, nospace, ...headingProps }) => {
  const sizeStyle = sizeStyles[size];
  const HTag = `h${rank}`;
  const className = cn(sizeStyle, { [styles.nospace]: nospace });
