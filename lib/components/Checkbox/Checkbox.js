...
function getCheckboxClassNames({ variant, disabled, isMobile }) {
  let classNames = [styles.checkboxState];
 ...
 * @constructor
 */
const Checkbox = forwardRef(({ variant, disabled, isMobile, ...props }, ref) => {
  const inputID = useMemo(() => generateId(), []);
  const classNames = getCheckboxClassNames({ variant, disabled, isMobile });

  return (
    <label ref={ref} htmlFor={inputID} className={isMobile ? styles.isMobileLabel : ''}>
      <div className={isMobile ? styles.mobileCheckboxContainer : styles.checkboxContainer}>
        <input
          id={inputID}
          type="checkbox"
          disabled={disabled}
