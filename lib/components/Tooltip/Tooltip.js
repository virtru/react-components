...
const TooltipWrapper = ({ children, ...tooltipProps }) => {
  ...
  useLayoutEffect(() => {
    if (isVisible) {
      ...
      if (tooltipProps.offsetTop) {
        top += tooltipProps.offsetTop;
      }
      if (tooltipProps.offsetLeft) {
        left += tooltipProps.offsetLeft;
      }
    ...
    tooltipProps.offsetTop,
    tooltipProps.offsetLeft,
  ]);
